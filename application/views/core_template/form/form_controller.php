...
class <?= ucwords($table_name); ?> extends Admin	
{
	...
	/**
	* show all <?= ucwords(clean_snake_case($table_name)); ?>
	*
	* @var $offset String
	*/
	public function index($offset = 0)
	{
		...
		$config = [
			'base_url'     => 'administrator/manage-form/{table_name}/index/',
		...
		$this->render('backend/standart/administrator/form_builder/{table_name}/{table_name}_list', $this->data);
	}

	/**
	* Update view <?= ucwords(clean_snake_case($table_name)); ?>
	*
	* @var $id String
	*/
	public function edit($id)
	{
		...
		$this->render('backend/standart/administrator/form_builder/{table_name}/{table_name}_update', $this->data);
	}

	/**
	* Update <?= ucwords(clean_snake_case($table_name)); ?>
	*
	* @var $id String
	*/
...
if (!empty(${table_name}_<?= $field_all[$file]['input_name']; ?>_uuid)) {
	...
	/**
	* delete <?= ucwords(clean_snake_case($table_name)); ?>
	*
	* @var $id String
	*/
	public function delete($id = null)
	{
		...
		if ($remove) {
            set_message(cclang('has_been_deleted', '<?= ucwords(clean_snake_case($table_name)); ?>'), 'success');
        } else {
            set_message(cclang('error_delete', '<?= ucwords(clean_snake_case($table_name)); ?>'), 'error');
        }
	...
	/**
	* View view <?= ucwords(clean_snake_case($table_name)); ?>
	*
	* @var $id String
	*/
	public function view($id)
	{
		...
		$this->render('backend/standart/administrator/form_builder/{table_name}/{table_name}_view', $this->data);
	}
	...
	*/
	public function get_<?= $field_all[$file]['input_name']; ?>_file($id)
	{
		...
		echo $this->get_file([
            ...
            'primary_key'       => '{primary_key}',
            'upload_path'       => 'uploads/{table_name}/',
            'delete_endpoint'   => 'administrator/{table_name}/delete_<?= $field_all[$file]['input_name']; ?>_file'
        ]);
	}
	<?php endforeach; ?>
